...
if( $url[2] == 'upload' && $u->info['admin'] > 0 ) {
	...
	if( $u->info['activ'] == 1 ) {
		...
		while($pl = mysql_fetch_array($sp)) {
			if( $url[4] == 'delete' && $url[5] == $pl['id'] ) {
				...
				$html .= '<a title="'.$usrs[$pl['uid']]."\n".date('d.m.Y H:i',$pl['time']).'" oncontextmenu="if(confirm(\'Вы уверены?\')){ top.location=\'http://xcombats.com/library/upload/'.htmlspecialchars($url[3],NULL,'cp1251').'/delete/'.$pl['id'].'/\'; }return false;" target="_blank" href="http://xcombats.com/ui/'.$pl['img'].'.'.$pl['type'].'"><img src="http://xcombats.com/ui/'.$pl['img'].'.'.$pl['type'].'" class="imgo"></a>';
			}
			$i++;
		}
		if( $i == 0 ) {
		 ...
		 if( isset($_FILES['filename']) && $u->info['admin'] > 0 ) {
			 ...
			 $html .= '
			  ...
			  <center>
			  <form action="http://xcombats.com/library/upload/" method="post" enctype="multipart/form-data">
			  <input type="file" name="filename"> 
			  <input class="btn2" type="submit" value="Загрузить"><hr>';
			  
			  if( $u->info['admin'] > 0 ) {
				 $html .= '<a href="http://xcombats.com/library/upload/all/">[ Все изображения ]</a> ';
			  }
			  $html .= '<a href="http://xcombats.com/library/upload/me/">[ Мои изображения ]</a>';
	...
	$i = 1;
	while($pl = mysql_fetch_array($sp)) {
		$html .= '<a target="_blank" href="http://xcombats.com/library/'.$pl['url_name'].'/">&gt;&gt; '.$pl['title'].'</a><br>Автор: '.$u->microLogin($pl['uid'],1).' / Дата публикации: '.date('d.m.Y',$pl['time']).'<hr>';
		$i++;
	}
	if( $html == '' ) {
		$html = 'В настоящий момент непроверенных статей нет.<br>
		<br>Если Вы хотите написать свою статью - <a target="_blank" href="http://xcombats.com/library/new/">http://xcombats.com/library/new/</a><br>
		<br>Более подробная информация - <a href="http://xcombats.com/library/public/">http://xcombats.com/library/public/</a>';
	}
...
?>
<!-- -->
<script src="http://new.xcombats.com/static/ckeditor/ckeditor.js"></script>
<!-- -->
...
?>
		...
		Ссылка для просмотра: <a target="_blank" href="http://xcombats.com/library/<?=$mbpage['url_name']?>/">http://xcombats.com/library/<?=$mbpage['url_name']?>/</a>
		<hr />
		Мы ценим проделанную Вами работу и постараемся как можно скорее рассмотреть заявку на добавление данной статьи
		<br /><br /><br /><br /><br /><br /><br /><br />
		, с уважением<br />
		Администрация Бойцовского Клуба "CombatZ".
<?
				}else{
	...
	?>
		...
		Ссылка для просмотра: <a target="_blank" href="http://xcombats.com/library/id<?=$sid?>/">http://xcombats.com/library/id<?=$sid?>/</a>
		<hr />
		Мы ценим проделанную Вами работу и постараемся как можно скорее рассмотреть заявку на добавление данной статьи
		<br /><br /><br /><br /><br /><br /><br /><br />
		, с уважением<br />
...
?>
<form method="post" action="http://xcombats.com/library/new/<?=$mbpage['url_name']?>">
<?
if( isset($mbpage['id']) ) {
	...
	if( isset($_POST['save2']) ) {
		...
		if( $u->info['admin'] > 0 ) {
			...
			if( $red500 == true ) {
				echo '<script>top.location.href="http://xcombats.com/library/new/'.$mbpage['url_name'].'/"</script>';
			}
		}
	...
	if( isset($pl['id']) ) {
		...
		if($pl['uid'] > 0 || $u->info['admin'] > 0) {
			...
			if($pl['uid'] == $u->info['id'] || $u->info['admin'] > 0) {
				echo ' &nbsp; / &nbsp; <a target="_blank" href="http://xcombats.com/library/new/'.$pl['url_name'].'/">Изменить</a>';
				if( $u->info['admin'] > 0 ) {
					echo ' &nbsp; / &nbsp; <a href="http://xcombats.com/library/'.$pl['url_name'].'/delete/">Удалить</a>';
				}
			}
			echo '</div></small>';
		}
