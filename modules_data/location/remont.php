...
if($u->room['file']=='remont')
{
	...
	}elseif(isset($_GET['remon']))
	{
		...
		if($rem_price<0.05){$rem_price=0.05;} //кэп поможет
		if($rem_all<0.05){$rem_all=0.05;} //кэп поможет
		...
		if(isset($ir['id']))
		{
			...
			if(isset($po['noremont']))
			{
				...
				if($rem_cell>$u->info['money'])
				{
					...
					if($upd)
					{
						mysql_query('UPDATE `dailybonus` SET `remont` = `remont` + "'.mysql_real_escape_string($rm).'" WHERE `uid` = "'.$u->info['id'].'" LIMIT 1');
						$il = $ir['iznosMAX'];
						...
						*/
						$polm = round(1 * ( $re2 / ($ir['iznosMAX']) ),4);
						if( $polm < 0 ) {
							$polm = 0;
						}elseif( $polm > 1 ) {
							$polm = 1;
	...
	}elseif(isset($_GET['remonz']))
	{
				...
				if($rem_cell>$u->info['money4'])
				{
					...
					if($upd)
					{
						...
						if( rand(0,100) < 11 ) {
							...
							$ir['iznosMAX'] -= ceil($ir['iznosMAX'])/100000*rand(10,700);
						}
						if($ir['iznosMAX']<1)
						{
							$ir['iznosMAX'] = 1;
		...
		if(isset($ir['id'])) {
			...
			if(isset($ur['id'])) {
				...
				if( $ur['price2'] == -1 ) {
					$ur['price2'] = $ui1['price2']-$ir['2price'];
				}
				if( $ur['price1'] == -1 ) {
					$ur['price1'] = $ui1['price1']-$ir['1price'];
				}
				//
				$po = $u->lookStats($ir['data']);
				//
				if(isset($po['toclan'])) {
					$re2 = 'Предмет принадлежит клану, вы не можете улучшить его.';
				}elseif(isset($po['sudba']) && ($po['sudba'] == '0' || $po['sudba'] == '1')) {
					$re2 = 'Предмет не связан с вами общей судьбой, наденьте его!';
				}elseif(isset($po['sudba']) && $po['sudba'] != $u->info['login']) {
					$re2 = 'Предмет не связан с вами общей судьбой!';
				}elseif($ur['price1'] > 0 && $u->info['money'] < $ur['price1']) {
					...
					if(isset($ui1['id'],$ui2['id'])) {
						...
						//
						if(isset($po['sudba'])) {
							$dopdata .= '|sudba='.$po['sudba'].'';
						}
						if(isset($po['nosale'])) {
							$dopdata .= '|nosale='.$po['nosale'].'';
						}
						if(isset($po['noremont'])) {
							$dopdata .= '|noremont='.$po['noremont'].'';
						}
						if(isset($po['frompisher'])) {
							$dopdata .= '|frompisher='.$po['frompisher'].'';
						}
						if(isset($po['notransfer'])) {
							$dopdata .= '|notransfer='.$po['notransfer'].'';
						}
						if(isset($po['fromshop'])) {
							$dopdata .= '|fromshop='.$po['fromshop'].'';
						}
						if(isset($po['maidin'])) {
							$dopdata .= '|maidin='.$po['maidin'].'';
						}
						//
						mysql_query('UPDATE `items_users` SET `item_id` = "'.$ui1['id'].'",`1price` = "'.$ui1['price1'].'",`2price` = "'.$ui1['price2'].'",`iznosMAX` = "'.$ui1['iznosMAXi'].'",`data` = "'.$ui2['data'].$dopdata.'" WHERE `id` = "'.$ir['id'].'" LIMIT 1');
						$re2 = 'Предмет &quot;'.$ir['name'].'&quot; был успешно улучшен до следующего уровня за '.$ur['price1'].' кр. и '.$ur['price2'].' екр.';
						$u->bank['money2'] -= $ur['price2'];
						$u->info['money'] -= $ur['price1'];
