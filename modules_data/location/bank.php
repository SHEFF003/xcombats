...
if($u->room['file']=='bank')
{
	$noc = $c['crtoecr']; //120 kr = 1 ekr.
	$con = $c['ecrtocr']; //1 екр. = 30 кр.
	function getNum($v)
	{
		$plid = $v;
	...
	{
		$bnklast = mysql_fetch_array(mysql_query('SELECT `id` FROM `bank` WHERE `uid` = "'.$u->info['id'].'" LIMIT 1'));
		if(isset($bnklast['id'])) {
			$re2 = 'Нельзя создавать более одного банковского счета!';
		}elseif( $_POST['rdn01'] == 2 && ($u->info['level'] >= 8 || $u->info['money4'] < 15 )) {
