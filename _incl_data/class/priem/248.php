...
if( isset($pr_momental_this)) {
	$fx_moment = function(  $uid, $enemy, $j_id, $yron, $profil ) {
		if(!isset($btl->stats[$btl->uids[$uid]]['um_priem'][$j_id])) {
			global $u, $btl;
			if( $profil >= 5 && $profil <= 11 ) {
				$yron = $yron/100*70;
				$btl->priemAddLogFast( $uid, 0, "Песчаный Щит",
		...
		if( $a > 0 ) {
			...
			while($j < count($at['p'][$a]['atack'])) {
				...
				$at['p'][$a]['atack'][$j][1] == 5 )) {
					if( !isset($at['p'][$a]['priems']['kill'][$uid][$j_id]) ) {
						//
						if( $at['p'][$a]['atack'][$j]['yron']['w_type'] >= 5 && $at['p'][$a]['atack'][$j]['yron']['w_type'] <= 11 ) { 
							$at['p'][$a]['atack'][$j]['yron']['y'] = round($at['p'][$a]['atack'][$j]['yron']['y']/100*70);
							$at['p'][$a]['atack'][$j]['yron']['r'] = round($at['p'][$a]['atack'][$j]['yron']['r']/100*70);
							$at['p'][$a]['atack'][$j]['yron']['k'] = round($at['p'][$a]['atack'][$j]['yron']['k']/100*70);
							$at['p'][$a]['atack'][$j]['yron']['m_k'] = round($at['p'][$a]['atack'][$j]['yron']['m_k']/100*70);
							$at['p'][$a]['atack'][$j]['yron']['m_y'] = round($at['p'][$a]['atack'][$j]['yron']['m_y']/100*70);
							//
