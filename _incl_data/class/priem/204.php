...
/*
	Прием: Обречённость
	Следующий нельзя заблокировать щитом, увернуться, парировать
*/
$pvr = array();
	...
	$fx_priem = function(  $id , $at , $uid, $j_id ) {
		...
		if( $a > 0 ) {
			...
			while($j < count($at['p'][$a]['atack'])) {
				if( $at['p'][$a]['atack'][$j][1] == 3 || (isset($at['p'][$a]['atack'][$j]['yron']) && (
				$at['p'][$a]['atack'][$j][1] == 1 ||
				$at['p'][$a]['atack'][$j][1] == 4 ||
				$at['p'][$a]['atack'][$j][1] == 5 )) ) {
					if( $pvr['used'] == 0 && !isset($at['p'][$a]['priems']['kill'][$uid][$j_id]) ) {
						...
						//
						$at['p'][$a]['atack'][$j]['yron']['plog'][] = '$this->deleffm(204,'.(0+$uid).','.$btl->stats[$btl->uids[$uid]]['u_priem'][$j_id][3].');
