...
/*
	Прием: Осторожность
	Следующий удар противника по вам нанесет на 50% меньше урона от магии
*/
$pvr = array();
	...
	$fx_moment = function(  $uid, $enemy, $j_id, $yron, $profil ) {
		if(!isset($btl->stats[$btl->uids[$uid]]['um_priem'][$j_id])) {
			...
			if( $profil > 4 && $profil < 12 ) {
				$yron = $yron/2;
				$btl->priemAddLogFast( $uid, 0, "Осторожность",
		...
		if( $a > 0 ) {
			...
			while($j < count($at['p'][$a]['atack'])) {
				...
				$at['p'][$a]['atack'][$j][1] == 5 )) {
					if( !isset($at['p'][$a]['priems']['kill'][$uid][$j_id]) ) {
						//
						if( $at['p'][$a]['atack'][$j]['yron']['w_type'] > 4 && $at['p'][$a]['atack'][$j]['yron']['w_type'] < 12 ) { 
