...
/*
	Прием: Заземление: Ноль
*/
$pvr = array();

$pvr['x5'] = mysql_fetch_array(mysql_query('SELECT `id`,`x` FROM `eff_users` WHERE `uid` = "'.$this->ue['id'].'" AND `v2` = 250 AND `delete` = 0 LIMIT 1'));
if( $pvr['x5']['x'] < 5 ) {
	$prv['x'] = '';
	if( $pvr['x5']['x'] > 0 ) {
		$prv['x'] = ' x'.($pvr['x5']['x']+1).'';
	...
	$btl->priemAddLog( $id, 1, 2, $u->info['id'], $this->ue['id'],
		'<font color^^^^#'.$prv['color2'].'>Заземление: Ноль'.$prv['x'].'</font>',
		$prv['text2'],
		($btl->hodID + 0)
	);
	...
	$cup = true;
	echo '<font color=red><b>На пероснаже достигнуто максиальное колличество заземлений</b></font>';
}
