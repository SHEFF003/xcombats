...
if( isset($pr_used_this) && isset($pr_moment) ) {
		...
		if( $a > 0 ) {
			...
			*/
			...
			$btl->priemAddLog( $id, 1, 2, $u2, $u1,
				'<font color^^^^#'.$prv['color2'].'>Проткнуть'.$prv['xx'].'</font>',
				$prv['text2'],
				($btl->hodID)
			);
		}
	...
	$btl->priemAddLog( $id, 1, 2, $u->info['id'], $this->ue['id'],
		'<font color^^^^#'.$prv['color2'].'>Проткнуть</font>',
		$prv['text2'],
		($btl->hodID + 1)
	);
	...
	//Добавляем прием
	//$this->addEffPr($pl,$id);
	$this->addPriem($this->ue['id'],$pl['id'],'atgm='.floor($pvr['hp']/10).'',0,77,10,$u->info['id'],1,'проткнуть',0,0,1);
