...
$pvr['x5'] = mysql_fetch_array(mysql_query('SELECT `id`,`x` FROM `eff_users` WHERE `uid` = "'.$u->info['id'].'" AND `v2` = 30 AND `delete` = 0 LIMIT 1'));
if( !isset($pvr['x5']['id']) ) {
	$prv['x'] = '';
	if( $pvr['x5']['x'] > 0 ) {
		$prv['x'] = ' x'.($pvr['x5']['x']+1).'';
	...
	$btl->priemAddLog( $id, 1, 2, $u->info['id'], 0,
		'<font color^^^^#'.$prv['color2'].'>Знак воздуха'.$prv['x'].'</font>',
		$prv['text2'],
		($btl->hodID + 0)
	);
