...
/*
	Прием: Жертва Воздуху
*/
$pvr = array();

$pvr['x5'] = mysql_fetch_array(mysql_query('SELECT `id`,`x` FROM `eff_users` WHERE `uid` = "'.$this->ue['id'].'" AND `v2` = 257 AND `delete` = 0 LIMIT 1'));
if( !isset($pvr['x5']['id']) ) {
	$prv['x'] = '';
	if( $pvr['x5']['x'] > 0 ) {
		$prv['x'] = ' x'.($pvr['x5']['x']+1).'';
	...
	if( $pvr['x5']['x'] > 0 ) {
		...
		//$pl['date3'] = 'add_mab1='.($u->info['level']*3).'|add_mab2='.($u->info['level']*3).'|add_mab3='.($u->info['level']*3).'|add_mab4='.($u->info['level']*3).'';
		$prv['eda'] = $pl['date3'];
	}
	//
	if( isset($prv['eda']) ) {
	...
	$btl->priemAddLog( $id, 1, 2, $u->info['id'], $this->ue['id'],
		'<font color^^^^#'.$prv['color2'].'>Жертва Воздуху'.$prv['x'].'</font>',
		$prv['text2'],
		($btl->hodID + 0)
	);
