...
if( isset($pr_used_this) && isset($pr_moment) ) {
	...
	$fx_priem = function(  $id , $at , $uid, $j_id ) {
		...
		if( $a > 0 ) {
			...
			/*
			Критует
			[18:56:22] Константин: //НЕ КРИТУЕТ! ИСПРАВЛЕНО!
			[18:56:23] Golden Fish: 100 процентов
			*/
			/*
			...
			$pvr['hp'] = $btl->testYronPriem( $btl->stats[$btl->uids[$u1]]['effects'][$prv['j_priem']]['user_use'], $u1, 12, $pvr['hp'], -1, true );
			...
			$btl->priemAddLog( $id, 1, 2, $u2, $u1,
				'<font color^^^^#'.$prv['color2'].'>Ядовитое Облако</font>',
				$prv['text2'],
				($btl->hodID)
			);
		}
	...
	$btl->priemAddLog( $id, 1, 2, $u->info['id'], $pvr['uid'],
		'<font color^^^^#'.$prv['color2'].'>Ядовитое Облако [8]</font>',
		$prv['text2'],
		($btl->hodID + 1)
	);
