...
/*
	Прием: Серое мастерство
*/
$pvr = array();

$pvr['x5'] = mysql_fetch_array(mysql_query('SELECT `id`,`x` FROM `eff_users` WHERE `uid` = "'.$u->info['id'].'" AND `v2` = 285 AND `delete` = 0 LIMIT 1'));
if( $pvr['x5']['x'] < 5 ) {
	$prv['x'] = '';
	if( $pvr['x5']['x'] > 0 ) {
		$prv['x'] = ' x'.($pvr['x5']['x']+1).'';
	...
	$btl->priemAddLog( $id, 1, 2, $u->info['id'], 0,
		'<font color^^^^#006699>Серое Мастерство'.$prv['x'].'</font>',
		$prv['text2'],
		($btl->hodID + 0)
	);
	...
	$cup = true;
	echo '<font color=red><b>На пероснаже достигнуто максиальное колличество заклинаний</b></font>';
}
