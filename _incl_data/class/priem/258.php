...
if( !isset($pvr['x5']['id']) ) {
	...
	//
	$this->addEffPr($pl,$id,'minprocmanahod=1x10');
	//
	$prv['effx'] = '';
	//
	$pl['date3'] = 'minprocmanahod=1x10';
	$prv['eda'] = $pl['date3'];
	//
	if( isset($prv['eda']) ) {
	...
	$prv['text2'] = '{tm1} '.$prv['text'].' на {u2}.'.$prv['effx'];
	$btl->priemAddLog( $id, 1, 2, $u->info['id'], $this->ue['id'],
		'<font color^^^^#'.$prv['color2'].'>Энергия Воздуха'.$prv['x'].'</font>',
		$prv['text2'],
		($btl->hodID + 0)
	);
