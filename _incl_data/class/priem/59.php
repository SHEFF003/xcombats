...
if(isset($pr_used_this) && isset($pr_moment)) {
	$fx_priem = function($id , $at , $uid, $j_id) {
		...
		if( $a > 0 ) {
			...
			//
			/*$pvr['hp'] = $priem->magatack( $u2, $u1, $pvr['hp'], 'огонь', 0 );
			...
			$btl->priemAddLog( $id, 1, 2, $u2, $u1,
				'<font color^^^^#'.$prv['color2'].'>Пожирающее Пламя [10]'.$prv['xx'].'</font>',
				$prv['text2'],
				($btl->hodID)
			);
		}
	...
	$btl->priemAddLog( $id, 1, 2, $u->info['id'], $this->ue['id'],
		'<font color^^^^#'.$prv['color2'].'>Пожирающее Пламя [10]</font>',
		$prv['text2'],
		($btl->hodID + 1)
	);
	...
	//Добавляем прием
	//$this->addEffPr($pl,$id);
	$this->addPriem($this->ue['id'],$pl['id'],'atgm='.floor($pvr['hp']/5).'',0,77,5,$u->info['id'],1,'пожирающеепламя',0,0,1);
