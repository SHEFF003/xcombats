...
class Filter {
	...
	public $abuse = array(
		"лох","мудак","гандон","пидарас","чмо","хуй","хуйн€","хуйни","шлюха","паскуда","бл€",'бл€ть','бл€дь',
	...
	public function e($t) {
		mysql_query('INSERT INTO `chat` (`text`,`city`,`to`,`type`,`new`,`time`) VALUES ("#'.date('d.m.Y').' %'.date('H:i:s').': <b>'.mysql_real_escape_string($t).'</b>","capitalcity","»громир","6","1","-1")');
	}
	...
	public function mystr($string)
	{
		...
		if(strtolower('S')!='s')
		{
			$ru  = 'јЅ¬√ƒ≈®∆«»… ЋћЌќ–ѕ—“”‘’÷„ЎўЏ№џЁёя';
			$ru2 = 'абвгдеЄжзийклмнорпстуфхцчшщъьыэю€';
	...
	{
		trigger_error('ƒублирование не допускаетс€.', E_USER_ERROR);
	}
}
